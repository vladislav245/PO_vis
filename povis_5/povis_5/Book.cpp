...
bool AD_Magazine::operator > (Book* obj) {

	int step = 0;

	int size = strlen(obj->topic);
	while (step != 5 && size > 0) {

		if (obj->topic[size - 1] < this->topic[step]) return 1;
		else if (obj->topic[size - 1] > this->topic[step]) return 0;
		else {
			step++;
			size--;
		}
